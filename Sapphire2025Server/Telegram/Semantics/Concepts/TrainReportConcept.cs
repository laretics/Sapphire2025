...
{
	public class TrainReportConcept:VerbConcept
	{
		...
		public override async Task<bool> match(string[] text)
		{
			...
			var keywords = new HashSet<string> { "INFORME","INFORMACION","ESTADO","DOSSIER","DATOS" };
			...
			if (found)
			{
				...
				if (await auxTrain.match(text)) ToTrain = auxTrain;
			}
			return found;
		}
	}
