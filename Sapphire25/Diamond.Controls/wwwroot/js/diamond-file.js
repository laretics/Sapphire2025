...
(function () {
	...
	function pickFileFallback(accept) {
		return new Promise(function (resolve) {
			...
			input.type = "file";
			input.accept = accept || ".ddm,.diamond,.txt,text/plain";
			...
			input.addEventListener("change", function () {
				...
				reader.onload = function () {
					document.body.removeChild(input);
					// Solo actualiza handle de script si es un script de demanda.
					var n = (file.name || "").toLowerCase();
					if (n.endsWith(".ddm") || n.endsWith(".diamond") || n.endsWith(".txt")) {
						clearHandle();
						fileName = file.name;
					}
					resolve({
	...
	window.diamondFile = {
		...
		open: async function () {
			if (supportsFsa()) {
				try {
					...
					return await pickFileFallback(".ddm,.diamond,.txt,text/plain");
				}
			}
			return await pickFileFallback(".ddm,.diamond,.txt,text/plain");
		},
		...
		/**
		 * Abre un archivo de texto genérico (p. ej. topología .xml). No cambia el handle del script.
		 * accept: atributo accept del input, p. ej. ".xml,application/xml,text/xml"
		 */
		openText: async function (accept, description) {
			var acceptAttr = accept || ".xml,application/xml,text/xml";
			var desc = description || "XML";
			if (supportsFsa()) {
				try {
					var handles = await window.showOpenFilePicker({
						multiple: false,
					...
					var text = await file.text();
					return { name: file.name || "topo.xml", content: text, hasHandle: false };
				} catch (e) {
					if (e && (e.name === "AbortError" || e.name === "NotAllowedError")) {
						return null;
					}
			...
			return await pickFileFallback(acceptAttr);
		},
