...
#pragma warning restore CA1002 // Do not expose generic lists
        ...
        public bool IsActive => TrainType == TrainType.Player;
...
#pragma warning restore CA1002 // Do not expose generic lists
        ...
        protected void LogTrainSpeed(double timeStamp)
        {
            ...
            if (lastLogTime + evaluationInterval >= timeStamp)
            {
                ...
                if ((evaluationContent & EvaluationLogContents.SignalAspect) == EvaluationLogContents.SignalAspect)
                {
                    //if (moveForward)
                    //{
                    //    builder.Append(NextSignalObjects[Direction.Forward]?.SignalLR(SignalFunction.Normal).ToString() ?? "-");
                    //}
                    //else
                    //{
                    //    builder.Append(NextSignalObjects[Direction.Backward]?.SignalLR(SignalFunction.Normal).ToString() ?? "-");
                    //}
                    builder.Append(Separator);
                }
        ...
        protected SignalAspectState GetNextSignalAspect(Direction direction)
        {
            ...
            //{
            //    thisAspect = NextSignalObjects[direction].SignalLR(SignalFunction.Normal);
            //}
...
//                TrainDeadlockInfo.CheckDeadlock(ValidRoutes[Direction.Forward], Number);
        ...
        protected virtual bool CheckEndOfRoutePosition()
        {
            ...
            if (TCRoute.ReversalInfo[TCRoute.ActiveSubPath].Valid || TCRoute.ActiveSubPath == TCRoute.TCRouteSubpaths.Count - 1)
            {
                ...
                if (!endOfRoute)
                {
                    ...
                    //     NextSignalObjects[Direction.Forward].SignalLR(SignalFunction.Normal) == SignalAspectState.Stop && TCRoute.ReversalInfo[TCRoute.ActiveSubPath].Valid)
                    //{
                    //    endOfRoute = true;
                    //}
                }
        ...
        public void RequestResetSignal(Direction direction)
        {
            ...
            //    {
            //        if (NextSignalObjects[direction]?.SignalLR(SignalFunction.Normal) != SignalAspectState.Stop)
            //        {
        ...
        internal void SetTrainOutOfControl(OutOfControlReason reason)
        {
            ...
            if (simulator.TimetableMode || reason != OutOfControlReason.OutOfPath || IsActualPlayerTrain)
            {
        ...
        public void UpdatePlayerTrainData(float maxDistanceM)
        {
            ...
            //        {
            //            TrackMonitorSignalAspect signalAspect = NextSignalObjects[Direction.Forward].TranslateTMAspect(NextSignalObjects[Direction.Forward].SignalLR(SignalFunction.Normal));
            //            SpeedInfo thisSpeedInfo = NextSignalObjects[Direction.Forward].SignalSpeed(SignalFunction.Normal);
        ...
        public void SetDoors(DoorSide side, bool open)
        {
            foreach (TrainCar car in Cars)
            {
                ...
                DoorSide carSide = side;// car.Flipped ? Doors.FlippedDoorSide(side) : side;
        ...
        public DoorState DoorState(DoorSide side)
        {
            return Cars.Select(car =>
            {
                ...
                var carSide = side;// car.Flipped ? Doors.FlippedDoorSide(side) : side;
                switch (carSide)
                {
                    case DoorSide.Left:
