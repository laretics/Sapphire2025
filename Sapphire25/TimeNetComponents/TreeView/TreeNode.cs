		...
		{ 
			get
			{
				switch (Type)
				{
					...
					case NodeType.Plan:
						if (null != NetEnvironment.Plan)
							return string.Format("{0} {2} ({1} circulaciones)", NetEnvironment.Plan.Name, NetEnvironment.Plan.CirculationCount, NetEnvironment.Plan.Id);
						return "[Unknown Plan]";
					case NodeType.Circulations:
						if (null != NetEnvironment.Plan)
							return string.Format("{0} circulaciones", NetEnvironment.Plan.CirculationCount);
			...
			get
			{
				...
				switch(Type)
				{
					...
					case NodeType.Axis:
						if(null!=NetEnvironment.Axis)
						{
							TopoAxis auxEje = NetEnvironment.Axis;
					...
					case NodeType.CirculationBlock:
						if(null!=NetEnvironment.Asimilation)
							auxColor = NetEnvironment.Asimilation.color[0];
						return string.Format("<svg xmlns=\"http://www.w3.org/2000/svg\" height=\"16px\" viewBox=\"0 -960 960 960\" width=\"16px\" fill=\"{0}\"><path d=\"M240-400v-80h480v80H240Zm0-120v-80h480v80H240Zm0-120v-80h480v80H240Z\"/></svg>", auxColor);
				}
