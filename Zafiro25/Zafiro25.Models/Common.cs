...
namespace Zafiro25.Models
{
	public static class Common
	{
		...
		static public string timeString(DateTime rhs)
		{
			...
			if (DateTime.Now.Subtract(rhs).Days < 1)
			{
				cadenaFormato = "Hoy, {0:HH:mm}";
			}
			else if (DateTime.Now.Subtract(rhs).Days < 2)
			{
				cadenaFormato = "Ayer, {0:HH:mm}";
			}
			else if (DateTime.Now.Subtract(rhs).Days > 30)
			{
				cadenaFormato = "{0:dd-MM-yy}";
			}
			return string.Format(cadenaFormato, rhs);
		}
	}
}
