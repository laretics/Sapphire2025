...
namespace Sapphire2025.Storage
{
	...
	{       
		...
		public async Task<UserModel?> Login(UserLoginModel data)
		{
            string jsonString = JsonSerializer.Serialize(data);
            HttpResponseMessage respuesta = await sendPutRequest("userlogin", jsonString);
            return await respuesta.Content.ReadFromJsonAsync<UserModel?>();
		}
        ...
        public static string userIconHtml(byte credentials, string color)
        {
            if (Utils.hasRole(credentials, Common.UserRole.Root))
