namespace Sapphire2025Models.I18n
{
	...
	public static class UiCatalog
	{
		...
		private static Dictionary<string, string[]> Build()
		{
			...
			A("topo.places", "Places Tourmaline", "Places Tourmaline", "Places Tourmaline");
			...
			A("rel.places.title",
				"Editor de places.xml de Tourmaline al magatzem Diamond",
				"Editor de places.xml de Tourmaline en el almacén Diamond",
				"Tourmaline places.xml editor in the Diamond store");
			A("rel.places.body",
				"Al magatzem Diamond hi ha una solapa Places Tourmaline per editar el catàleg de llocs (places.xml) que usa Tourmaline.\n\nPodeu revisar el format abans de desar: si hi ha errors, el document no s'actualitza i es mostra la llista per corregir-los. El tren descarrega el catàleg quan canvia el hash.",
				"En el almacén Diamond hay una solapa Places Tourmaline para editar el catálogo de lugares (places.xml) que usa Tourmaline.\n\nPuede revisar el formato antes de guardar: si hay errores, el documento no se actualiza y se muestra la lista para corregirlos. El tren descarga el catálogo cuando cambia el hash.",
				"The Diamond store has a Places Tourmaline tab to edit the places catalog (places.xml) used by Tourmaline.\n\nYou can check the format before saving: if there are errors, the document is not updated and a list is shown so you can fix them. The train downloads the catalog when the hash changes.");
